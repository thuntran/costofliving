...
#' of a single person earning minimum wage at one or more states, given a vector
#' of states as input.
#'
#'
#' @import dplyr
...
avg_col_multiple_states_plot <- function(states) {
  # Input is only valid with 2 or more states
  if (length(states) == 1) {
  ...
  plot <- ggplot(table, aes(x = net_salary_avg_col_difference, y = factor(state_abbr))) +
    ...
    labs(
      title = "Difference in Average Cost of Living and Minimum Wage Net Salaries\nAcross Some U.S States",
      x = "Difference in Average Cost of Living and Minimum Wage Net Salaries (Dollars)",
      y = "States"
    ) +
