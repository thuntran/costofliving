...
test_that("Testing avg_col_multiple_states_plot(c(\"MA\",\"TX\",\"CA\", \"MA\"))", {
  ...
  expect_error(avg_col_multiple_states_plot(c("MA", "TX", "CA", "MA")), expected_error_message)
})

test_that("Testing avg_col_multiple_states_plot(\"MA\")", {
  expected_error_message <- "Please enter 2 or more states to proceed."
  expect_error(avg_col_multiple_states_plot("MA"), expected_error_message)
})
