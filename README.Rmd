...
## Functions Included
  + `gross_salary`: A function that calculates the annual gross salary of a single person earning minimum wage at a specific state.
  + `federal_tax_owed`: A function that calculates the amount of federal tax owed by an individual earning minimum wage based on federal income tax brackets.
  + `state_local_tax_owed`: A function that calculates the amount of state and local tax owed by an individual earning minimum wage.
  + `net_salary`: A function that calculates the combined tax amount (federal, state, local taxes) for by an individual earning minimum wage, and obtains the net salary by deducting from the gross salary.
  + `net_salary_avg_col_difference`: A function that calculates the difference between the average costs of living by the net salary.
  + `avg_col_multiple_states`: A function that calculates the annual gross salary, total tax (federal, state, local), net salary, and difference between average cost of living and net salary of a single person earning minimum wage at one or more states.
  + `avg_col_multiple_states_plot`: A function that plots the difference in average cost of living and net salary of a single person earning minimum wage at one or more states.
  + `avg_col_multiple_states_plot_all`: A function that plots the difference in average cost of living and net salary of a single person earning minimum wage across all states in the USA.
  + `avg_col_breakdown_plot`: A function that plots the average cost of living for a single person at a specific state.
